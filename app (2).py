...
The scaler and per-song "Mood" label aren't shipped as separate files --
they're computed once at startup (cached) directly from the raw dataset,
using the same mood-profile logic as the notebook.
"""
...
@st.cache_data
def load_and_prepare_song_data():
    """Load the raw Spotify dataset, clean it, fit a scaler, and assign each
    song a Mood by nearest mood-profile similarity -- same logic as the
    notebook, just run once here instead of shipped as separate files."""
    df = pd.read_csv(DATA_PATH)

    keep_cols = [c for c in (["Track Name", "Artist Name(s)", "Popularity"] + FEATURES) if c in df.columns]
    df = df[keep_cols].dropna(subset=[f for f in FEATURES if f in df.columns])
    df = df.reset_index(drop=True)

    scaler = StandardScaler()
    song_scaled = scaler.fit_transform(df[FEATURES].to_numpy())

    mood_names = list(MOOD_PROFILES.keys())
    mood_matrix = np.array([[MOOD_PROFILES[m][f] for f in FEATURES] for m in mood_names])
    mood_matrix_scaled = scaler.transform(mood_matrix)

    similarity = cosine_similarity(song_scaled, mood_matrix_scaled)
    best_idx = np.argmax(similarity, axis=1)
    df["Mood"] = [mood_names[i] for i in best_idx]
    df["Mood_Similarity"] = similarity[np.arange(len(df)), best_idx]
...
if missing:
    st.error(
        ...
        ". Make sure the model and dataset are in the repo at the "
...
spotify, scaler = load_and_prepare_song_data()
